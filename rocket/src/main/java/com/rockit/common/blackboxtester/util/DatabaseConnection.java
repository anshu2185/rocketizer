...
public class DatabaseConnection {
	...
	protected void createDatabaseConnection() {
		...
		try {
			if(this.dbUrl.contains("db2")) {
				Class.forName(Constants.DB2_DRIVER).newInstance();	
				this.connection = DriverManager.getConnection(this.dbUrl, this.dbUser, this.dbPwd);
			}
			if(this.dbUrl.contains("oracle")) {
				Class.forName(Constants.ORACLE_DRIVER).newInstance();	
				this.connection = DriverManager.getConnection(this.dbUrl, this.dbUser, this.dbPwd);
			}
